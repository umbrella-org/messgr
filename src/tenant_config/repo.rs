...
/// Inserts or overwrites the tenant's single `tenant_config` row, inside a
/// caller-owned transaction so it commits atomically with `load_tx`'s read
/// and `lock_tx`'s serialization (T-026). No non-transactional sibling:
/// `configure::set_tenant_config_inner` is the only caller, and it always
/// needs that atomicity. Not itself responsible for the
/// created/updated/idempotent distinction — the caller decides that from
/// `load_tx` and only calls this when the values actually differ or no row
/// exists.
