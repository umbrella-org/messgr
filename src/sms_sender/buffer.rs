...
//! against Postgres (decision 9): the OTP send already completed one way or
//! the other by the time this runs, so the write is retried out-of-band
...
pub async fn drain(
    ...
    let contents = match std::fs::read_to_string(path) {
        ...
        Err(err) => {
            tracing::error!(%err, "sms-sender: reading the buffer file failed");
            return;
        }
    };
    ...
    for line in contents.lines().filter(|line| !line.trim().is_empty()) {
        let record: AuditRecord = match serde_json::from_str(line) {
            ...
            Err(err) => {
                tracing::error!(
                    %err,
                    "sms-sender: buffer file contains an unparsable record, dropping it"
                );
                continue;
            }
        };
            ...
            .await
        {
            ...
            Err(err) => {
                tracing::error!(
                    ...
                    "sms-sender: buffer drain could not open the tenant pool, keeping for next pass"
                );
        ...
        if let Err(err) = repo::write_audit_record(&tenant.pool, &record).await {
            tracing::error!(
                ...
                "sms-sender: buffer drain write failed, keeping for next pass"
            );
            remaining.push(record);
        }
    }
    ...
    if let Err(err) = std::fs::write(path, rewritten) {
        tracing::error!(%err, "sms-sender: rewriting the buffer file after drain failed");
    }
}
