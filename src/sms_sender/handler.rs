...
use super::model::{
    AuditRecord, PendingAuditRecord, SendOtpRequest, SendOtpResponse, SmsSenderError,
};
...
pub async fn send_otp(
    ...
    // `tenant.auth_enabled` fail-open check (decision 8) -- ahead of the
    // provider call, the same cheapest-first ordering `messgr-ingest`'s
    // kill-switch check uses: a disabled tenant never reaches the provider
    // at all. Destination is never hashed or encrypted for a discarded row
    // -- empty `bytea` values satisfy the NOT NULL columns without implying
    // any lookup identity for them.
    ...
    // audit record (F1 rework): the entire point of this path is that a
    // Vault or Postgres outage must never block the OTP itself reaching the
    // customer (AGENTS.md hard invariant 1, decision 9's "the send still
    // succeeds"). `payload_ciphertext` is always NULL (decision 5) --
    // `body.body` (the OTP code) is used transiently here and never
    // persisted anywhere, encrypted or not (it is a live credential, §7.4).
    // Only the audit trail below, not the send itself, degrades to
    // best-effort.
...
/// and writes (or buffers) the completed `AuditRecord` -- exactly what the
/// handler used to do inline, before the provider call, with a bare `?`
/// aborting the whole request on a Vault/Postgres hiccup (F1). That crypto
/// step now runs *after* the send and, on its own failure, buffers a
    ...
    .await
    {
        ...
        Err(err) => {
            tracing::error!(
                %err,
                %comms_request_id,
                "sms-sender: DEK resolution failed, buffering the audit record for later encryption"
            );
            return None;
        }
    };
    ...
    match encryption::encrypt(&dek, aad, destination.as_bytes()) {
        ...
        Err(err) => {
            tracing::error!(
                %err,
                %comms_request_id,
                "sms-sender: encrypting the destination failed, buffering the audit record for later encryption"
            );
            None
        }
    }
