...
/// Fixed for every request this binary handles (mirrors
/// `sms_sender::model`'s reasoning -- decision 4 there: hardcoded, not
/// imported from `ingest::model::class`).
pub const CHANNEL: &str = "sms";
pub const CLASS: &str = "auth";
/// The sentinel template approved once per tenant via
...
/// drain writes byte-identical values (id-idempotency).
...
/// calls the provider before any of this), so this is buffered
...
/// exists for -- it is never written to disk in plaintext.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
pub enum OtpError {
    ...
    TenantNotActive,
    TenantNotConfigured,
    /// `tenant.auth_enabled` is `false` -- a discarded record was already
...
impl From<ResolutionError> for OtpError {
...
impl From<RegistryError> for OtpError {
...
impl From<EncryptionError> for OtpError {
    fn from(err: EncryptionError) -> Self {
        Self::Encryption(err)
    }
}

impl From<KeyStoreError> for OtpError {
    fn from(err: KeyStoreError) -> Self {
        Self::Vault(err)
    }
}

impl From<CustomerDekError> for OtpError {
...
impl From<ProviderSendError> for OtpError {
