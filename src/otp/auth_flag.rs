//! `tenant.auth_enabled`'s reader on the cloud OTP path. One process-wide
//! cache, refreshed every 5 seconds against the control database directly --
//! not a per-tenant poll task and not `kill_switch::cache::KillSwitchCache`
//! (shape mirrored, not reused: `auth_enabled` is one query against
//! `control_pool`, with no per-tenant database to open). A row missing from
//! the last successful refresh, or a refresh that itself failed, reads as
//! enabled -- the fail-open mandate ("a control-database outage must never
...
impl AuthEnabledCache {
    ...
    /// provisioned tenant not seen yet -- fail open.
...
/// control-DB poll, not a per-tenant-DB one. A refresh error is logged and
...
pub async fn run_refresh_loop(
    ...
    loop {
        if let Err(err) = cache.refresh(&control_pool).await {
            tracing::error!(%err, "otp: auth_enabled cache refresh failed");
        }
