...
//! was unreachable -- the OTP itself was already sent by the time this runs
//! (`handler::send_otp` calls the provider first), so like `buffer.rs`'s own
//! DB-write buffer this is retried out-of-band rather than failing the
//! request or blocking on the outage.
...
/// this buffer exists because just failed. Domain-separated from
/// `destination_hmac::compute`'s own use of the same pepper, and from
/// `sms_sender::pending::derive_key`'s label, by the label below -- this
/// binary and on-prem `sms-sender` never produce comparable output from the
/// same pepper. Returned `Zeroizing`, matching every other unwrapped
/// key-material value in this codebase.
pub fn derive_key(pepper: &[u8]) -> Zeroizing<Vec<u8>> {
    let mut mac = <Hmac<Sha256>>::new_from_slice(pepper)
        .expect("HMAC accepts a key of any length");
    mac.update(b"messgr-otp:pending-buffer-destination-key");
...
pub async fn drain(
    ...
    let contents = match std::fs::read_to_string(path) {
        ...
        Err(err) => {
            tracing::error!(%err, "otp: reading the pending-crypto buffer file failed");
            return;
        }
    };
    ...
    for line in contents.lines().filter(|line| !line.trim().is_empty()) {
        let record: PendingAuditRecord = match serde_json::from_str(line) {
            ...
            Err(err) => {
                tracing::error!(
                    %err,
                    "otp: pending-crypto buffer file contains an unparsable record, dropping it"
                );
                continue;
            }
        };
            ...
            .await
        {
            ...
            Err(err) => {
                tracing::error!(
                    ...
                    "otp: pending-crypto drain could not open the tenant pool, keeping for next pass"
                );
        ...
        .await
        {
            ...
            Err(err) => {
                tracing::error!(
                    ...
                    "otp: pending-crypto drain could not resolve the DEK, keeping for next pass"
                );
                remaining.push(record);
                continue;
            }
        ...
        let destination_bytes = match encryption::decrypt(
            ...
            Err(err) => {
                tracing::error!(
                    %err,
                    comms_request_id = %record.comms_request_id,
                    "otp: pending-crypto drain could not decrypt the buffered destination, keeping for next pass"
                );
                remaining.push(record);
                continue;
            }
        ...
        let destination = match String::from_utf8(destination_bytes) {
            ...
            Err(err) => {
                tracing::error!(
                    %err,
                    comms_request_id = %record.comms_request_id,
                    "otp: pending-crypto drain decrypted a non-UTF-8 destination, keeping for next pass"
                );
                remaining.push(record);
                continue;
            }
        ...
        let destination_ciphertext = match encryption::encrypt(
            ...
            Err(err) => {
                tracing::error!(
                    %err,
                    comms_request_id = %record.comms_request_id,
                    "otp: pending-crypto drain encryption failed, keeping for next pass"
                );
                remaining.push(record);
                continue;
            }
        ...
        if let Err(err) = repo::write_audit_record(&tenant.pool, &audit_record).await {
            tracing::error!(
                ...
                "otp: pending-crypto drain resolved crypto but the write failed, buffering to the write-retry buffer"
            );
            if let Err(io_err) = buffer::append(buffer_path, &audit_record) {
                tracing::error!(
                    %io_err,
                    comms_request_id = %audit_record.comms_request_id,
                    "otp: buffering the recovered record also failed -- record is lost"
                );
            }
        }
    }
    ...
    if let Err(err) = std::fs::write(path, rewritten) {
        tracing::error!(%err, "otp: rewriting the pending-crypto buffer file after drain failed");
    }
}
