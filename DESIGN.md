...
0. Vault cluster: 3-node Raft, Transit engine, AppRole auth, unseal runbook with named keyholders. Blocks step 1.
0b. Control database, tenant registry, `producer_cert` mapping, and the single-command provisioning script (§4.11, §11.4). Even for the on-prem launch customer — provisioning that one tenant through the same path is what keeps on-prem and cloud from diverging. `tenant_id` is present from the first migration; retrofitting a tenant column across a partitioned ledger later is a rewrite.
1. Producer registry + mTLS identity (§4.9). First, because `producer_id` is on the ledger from row one and every later control keys on it.
2. Ledger and outbox schema, ingest API, one channel (SMS), no gates. **Envelope encryption via Vault Transit included from the first write** — see note below. Prove the queue mechanics end to end.
3. Customer projection + resolution at ingest (§4.6–4.8), provisional shells, alias expansion. Event feed consumer can be stubbed initially; the schema and resolution path cannot.
4. Kill switches (§5.2). Early and deliberately — this is the control you want in place *before* the system can send at volume, not after the first incident proves you need it.
5. Verification, consent, and suppression gates. Regulatory blockers, not enhancements.
6. Producer quotas + `producer_usage` flush (§5.1).
7. Dispatcher HA, circuit breakers, retry and backoff.
8. Quiet hours with jitter and DST handling.
9. Scheduled delivery, cancellation, and `expires_at` (§6.2).
10. Customer event feed consumer, replacing the step-3 stub. Nightly reconciliation job.
11. Remaining channels (email, WhatsApp) behind the same `Sender` trait.
12. Webhook receiver and delivery-receipt ingestion.
13. Query API and UI, with `AuthProvider` trait + `MockProvider` + the production-guard check (§11.1). Real OIDC wiring lands whenever the IdP is available; nothing else blocks on it.
14. Admin panel: quota dashboard, kill-switch console, scheduled queue (§11.3).
15. Erasure tooling: crypto-shred first, then physical redaction and the erasure audit log.
16. SMS provider failover (§12.1) — before step 17, not after.
17. OTP fast path. Last, because it touches the auth flow and should not be the thing shaking out bugs in the sender adapters.
18. Bulk campaign path and ingest admission rate limiting.
19. **Cloud enablement**, once the on-prem customer is live: `messgr-otp` (§3.1), platform console (§11.4), platform kill switches, offboarding modes (§7.7), second region. Everything before this point already runs multi-tenant at N=1, so this step adds surfaces rather than reworking the core.
