...
`status=pending`. As of this writing the only other values the dispatcher actually writes are
`sent`, `failed`, and `expired` -- DESIGN.md §4.4 documents a larger eventual vocabulary
(`delivered`, `bounced`, `cancelled`, `suppressed_consent`, `suppressed_list`,
