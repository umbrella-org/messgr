...
with no match has its `reconcile_attempts` counter incremented and is deleted once that counter
reaches a fixed cap of 5. Unlike idempotency-sweep and partition-lifecycle, this command does
connect to Vault -- promotion is the payload's first write under a customer scope. Safe to re-run;
meant to be invoked on a schedule (cron/systemd timer), not run continuously -- this binary does
not daemonize.
