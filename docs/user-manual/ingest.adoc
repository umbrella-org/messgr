...
combination is rejected (`422`). No gate chain exists yet (consent, quotas, kill switches,
suppression -- §5, §5.1, §5.2 -- are unbuilt; `T-016` is kill switches specifically, not the gate
chain as a whole), and `class = "auth"` is rejected outright (`422`) -- OTP has its own path
