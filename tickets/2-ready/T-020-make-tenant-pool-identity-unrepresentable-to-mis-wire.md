...
Same mechanical edit in `tests/tenancy.rs`, `tests/kill_switch.rs`, `tests/dispatcher.rs`, `tests/ingest.rs`, `tests/tenant_config.rs`, `tests/customer.rs`, `tests/ledger_outbox_schema.rs`, `tests/partition_lifecycle.rs`, `tests/customer_dek.rs`, `tests/stats.rs`, `tests/provider_config.rs`. Each fixture already calls `provision_tenant` immediately before `connect_tenant_pool` — capture its returned `ProvisionOutcome.tenant_id` (currently discarded in several fixtures) and thread it through; add a `tenant_id: Uuid` field to any fixture struct (`TestTenant`, `Fixture`, …) that reconnects later in the same test.
