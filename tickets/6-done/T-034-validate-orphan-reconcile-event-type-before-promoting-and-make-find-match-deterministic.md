...
### Rework fix record — round 1 (commit be1a90c)

F1 fixed: `docs/user-manual/control-plane-cli.adoc`'s "Orphan-event reconciliation" paragraph now
reads "a row with no match, or whose `event_type` is outside the set DESIGN.md §4.4 documents
(never checked against `comms_event` at all in that case), has its `reconcile_attempts` counter
incremented and is deleted once that counter reaches a fixed cap of 5" — folding the new age-out
reason into the same sentence rather than adding a second one, matching the doc's existing style.
`just docs-check` reran clean.

### Scoped re-review (round 1)

Read `git show be1a90c` in full — a single-file, single-clause change to prose, no code. F1's
gap is closed: the sentence no longer implies "no match" is the only reason a row disappears.
Checked the new clause itself for new defects: it correctly cross-references DESIGN.md §4.4 (the
same section `is_recognized_event_type`'s own doc comment in `reconcile.rs` points at), and its
parenthetical ("never checked against `comms_event` at all in that case") is accurate — Task 1's
gate skips `find_match` entirely for an unrecognized `event_type`, it doesn't call it and reject
the result. No new finding. No other rework-round diff to read (this round fixed F1 alone, per
scope).
